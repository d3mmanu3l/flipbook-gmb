...
var bookmarkName = '_bookmark_' + location.href.substr(location.host.length);
...
var sliderSettings = {}, nav = {}, launcherConfig = {layouts:[
 	{
...
$(function(){
	...
	if($('ul.thumbs').length) $('#flipboardfooter').hide();
	$sl = $('#slider');
	initPageMode();
...
function addNavProps(){
	...
	nav.highlight = function(n,c,q) {
		...
		var rect = $('<div role="presentation" class="pageItem flip-highlight">&nbsp</div>');
...
function getStartLayout(){
	var numLayouts = launcherConfig.layouts.length;
	if(numLayouts === 1) { return 0;
	} else if(numLayouts === 2 && launcherConfig.layouts[1].trigger.toString().match(/(portrait|landscape)/)){
		return (launcherConfig.layouts[1].trigger === getOrientation()) ? 1 : 0;
	} else {
		var sorted = launcherConfig.layouts.sort(function(a,b){
...
function initMedia(hasSlider){
	if(isBaker) return;
	if(!$('video,audio').length && launcherConfig.layouts.length < 2) {
